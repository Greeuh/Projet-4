...
function unreportComment($commentId)
{
    ...
    $unreportComment = $commentManager->unreportComment($commentId);

    if ($unreportComment === false) {
        throw new Exception('Impossible d\'enlever le signalement de ce commentaire !');
    } else {
        header('Location: admin.php?action=showReportedComments');
    }
}
